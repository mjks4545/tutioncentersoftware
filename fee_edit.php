...
$idget=$_GET['id'];
$sql = ("SELECT * FROM `fee` WHERE `s.no` = '$idget'");
...
if( isset( $_POST['submit'] ) ){
	...
	//data insertion 
$sql = ("UPDATE `fee` SET `name` = '$name', `total_fee` = '$total_fee', `received` ='$received' WHERE `s.no` = '$idget';");
$stmt = $conn->prepare($sql);
$stmt->execute();
                ...
                <form role="form" method="post">
                  <div class="box-body">
                    ...
                    <div class="col-md-6">  <label for="exampleInputPassword1">total fee</label>
                      <input type="text" name="total_fee" class="form-control" id="exampleInputPassword1" placeholder="enter total fee" value="<?=$result[0]['total_fee']?>">
                      </div>
					  <div class="col-md-6"></div>
				   </div>
					 <div class="form-group col-md-12">
